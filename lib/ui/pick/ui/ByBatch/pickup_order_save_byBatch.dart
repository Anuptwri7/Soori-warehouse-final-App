...
import 'package:soori_warehouse/ui/pick/ui/ByBatch/testPickUpByBatch.dart';
...
log('https://$finalUrl${StringConst.urlCustomerOrderApp}pack-type?&purchase_detail=$receivedOrderID&location_code=');
log(response.body.toString());
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        for (int i = 0; i < data.length; i++) {
          if (location.contains(data[i].locationCode.toString())) {
            ...
            log(data[i].locationCode.toString());
          }
        }
  ...
  void initState() {
    super.initState();
    log(widget.orderNo+'    '+widget.orderId.toString()+'   '  +widget.purchaseDetail.toString());
    availableLocation(
  ...
  displayLocation() {
    return Padding(
      padding: const EdgeInsets.all(12.0),
      child: smallShowMorePickUpLocations("${_currentScannedLocation.isEmpty?"Scan the Location":location.join("\n").toString()} "),
    );
  }
